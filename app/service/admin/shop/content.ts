...
export default class ShopContentService extends BaseService {
  ...
  async page(query: QueryContentDto) {
    const { limit, title, page, startTime, endTime, id, uuid, lang, status } =
      query;

    const result = await this.getRepo()
      .admin.shop.Content.createQueryBuilder("content")
  ...
  async info(query: InfoContentDto) {
    const _query = query.id
      ? { id: query.id }
      : { identifier: query.identifier, lang: query.lang };
