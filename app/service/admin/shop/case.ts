...
export default class ShopCaseService extends BaseService {
  ...
  async list(query: QueryCaseDto) {
    ...
    const result = await this.getRepo()
      ...
      .getMany();
    const resultX: Array<CreateCaseDto> = result.map((d) => {
      return plainToClass(CreateCaseDto, d, { strategy: "excludeAll" });
    });
    return resultX;
  }
  ...
  async listByCaseClassUrl(url: string) {
    const caseClassModel: ShopCase =
      await this.service.admin.shop.caseClass.getModelByUrl(url);
    let result: Array<CreateCaseDto> = [];
    if (!_.isEmpty(caseClassModel)) {
      result = await this.list(
        plainToClass(QueryCaseDto, { categoryId: caseClassModel.id })
      );
    }
    return {
      caseList: result,
  ...
  async infoByUrl(url: string, lang: string) {
    const model: any = await this.getRepo().admin.shop.Case.findOne({
      url,
      lang,
    });
    if (_.isEmpty(model)) {
